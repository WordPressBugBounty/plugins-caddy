...
?>
<form name="caddy-form" id="caddy-welcome-message-form" method="post" action="">
	<?php wp_nonce_field('caddy-welcome-message-settings-save', 'caddy_welcome_message_settings_nonce'); ?>
	<input type="hidden" name="cc_submit_hidden" value="Y">
	<div class="cc-settings-container">
		<h2><span class="section-icons"><img src="<?php echo plugin_dir_url( CADDY_PLUGIN_FILE ) . 'admin/img/icon-welcome.svg'; ?>" /></span>&nbsp;<?php echo esc_html( __( 'Welcome Message', 'caddy' ) ); ?></h2>
		<p><?php echo esc_html( __( 'Customize the welcome message within the Caddy front-end.', 'caddy' ) ); ?></p>
		<?php if ( $caddy_license_status !== 'valid' ) { ?>
			<div class="cc-box cc-box-cta cc-upgrade">
				...
				<h3><?php echo esc_html( __( 'Unlock Custom Welcome Messages with Caddy Pro', 'caddy' ) ); ?></h3>
				<p><?php echo esc_html( __( 'Customize the welcome message within the Caddy front-end. Plus:', 'caddy' ) ); ?></p>
				<ul>
		...
		<?php if ( $caddy_license_status == 'valid' ) { ?>
			<table class="form-table">
				<tbody>
					<?php do_action( 'caddy_welcome_settings_start' ); ?>
					<?php do_action( 'caddy_welcome_settings_end' ); ?>
				</tbody>
			</table>
		<?php } ?>
	</div>
