...
?>
<form name="caddy-form" id="caddy-announcement-bar-form" method="post" action="">
	<?php wp_nonce_field('caddy-announcement-bar-settings-save', 'caddy_announcement_bar_settings_nonce'); ?>
	<input type="hidden" name="cc_submit_hidden" value="Y">
	<div class="cc-settings-container">
		<h2><span class="section-icons"><img src="<?php echo plugin_dir_url( CADDY_PLUGIN_FILE ) . 'admin/img/icon-announcement.svg'; ?>" /></span>&nbsp;<?php echo esc_html( __( 'Announcement Bar', 'caddy' ) ); ?></h2>
		<p><?php esc_html_e('Customize the announcement text that shows in the Caddy cart.', 'caddy-premium'); ?></p>
		<?php if ( $caddy_license_status !== 'valid' ) { ?>
			<div class="cc-box cc-box-cta cc-upgrade">
				...
				<h3><?php echo esc_html( __( 'Unlock an Announcement Bar with Caddy Pro', 'caddy' ) ); ?></h3>
				<p><?php echo esc_html( __( 'Display a targeted announcement bar to shoppers. Plus:', 'caddy' ) ); ?></p>
				<ul>
		...
		<?php if ( $caddy_license_status == 'valid' ) { ?>
			<table class="form-table">
				<tbody>
					<?php do_action( 'caddy_announcement_bar_settings_start' ); ?>
					<?php do_action( 'caddy_announcement_bar_settings_end' ); ?> 
				</tbody>
			</table>
		<?php } ?>
	</div>
